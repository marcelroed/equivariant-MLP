...
class Rep(object):
    ...
    def __eq__(self, other):
        ...
        d1 = tuple(
            [(k, v) for k, v in self.__dict__.items() if (k not in ['_size', 'is_permutation', 'is_orthogonal'])])
        d2 = tuple(
            [(k, v) for k, v in other.__dict__.items() if (k not in ['_size', 'is_permutation', 'is_orthogonal'])])
        return d1 == d2
...
def sparsify_basis(Q, lr=1e-2):  # (n,r)
    ...
    def loss(W):
        return jnp.abs(Q @ W.T).mean() + .1 * (jnp.abs(W.T @ W - jnp.eye(W.shape[0]))).mean() + .01 * \
               jax.numpy.linalg.slogdet(W)[1] ** 2
