...
class ZksZnxZn(Group):
    ...
    def __init__(self, k, n):
        ...
        Ik = I(k)
        assert k in [2, 4]
        self.discrete_generators = [LazyKron([Ik, nshift, In]), LazyKron([Ik, In, nshift]),
...
class DirectProduct(Group):
    def __init__(self, G1, G2):
        I1, I2 = I(G1.d), I(G2.d)
        self.lie_algebra = [LazyKronsum([A1, 0 * I2]) for A1 in G1.lie_algebra] + [LazyKronsum([0 * I1, A2]) for A2 in
                                                                                   G2.lie_algebra]
        self.discrete_generators = [LazyKron([M1, I2]) for M1 in G1.discrete_generators] + [LazyKron([I1, M2]) for M2 in
                                                                                            G2.discrete_generators]
